...
- `src/components/hero.tsx` — the scroll-driven hero. Structure: an oversized wrapper (`h-[300vh]`) containing a `sticky top-0 h-screen` viewport; `framer-motion`'s `useScroll`/`useTransform` map scroll progress within that wrapper to: photo opacity/scale, a warm tint overlay opacity, the brand mark's scale/opacity/rotation (growing from a small accent into an abstract frame-filling texture), and the copy's opacity/translate (fades out early). The photo uses a plain `<picture>` with breakpoint `<source>`s (not `next/image` art-direction, which doesn't support it) pointing at the three pre-cropped files in `public/images/`. A `HeroHighlights` list (3 short items — "Atendimento online/presencial", "Sigilo profissional", all placeholder pending confirmation) is pinned to the true right viewport edge, vertically centered (`lg:` only) via its own absolutely-positioned layer, independent of the centered copy container — it fills the side of the screen the left-of-center headline leaves bare at wide viewports. Fades on the same scroll schedule as the main copy. Kept in Hero's original flat/typographic treatment (no glass/blob), consistent with Hero staying outside the Refined Organicism restyle below.
